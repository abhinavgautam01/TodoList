        ...
        <Grid item md={8} px={4}>
            ...
            <Grid container display="flex" justifyContent="center">
                ...
                <Grid item display="flex" flexDirection="column" xs={10} md={8}>
                    ...
                    {!error && Array.isArray(data) && data.length === 0 && <Alert severity="warning">
                        You do not have any tasks created yet. Start by creating some tasks
                    </Alert>}
                    {isPending ? <LinearProgress/>: (Array.isArray(data) && data.length>0 && data.map((each, index)=>{return each.status===Status.todo || each.status===Status.inProgress ? (<Task key={index + each.priority} id={each.id} title={each.title} date={new Date(each.date)} description={each.description} priority={each.priority} status={each.status} onStatusChange={onStatusChangeHandler} onClick={markCompleteHandler} />) : (false)}) )}
